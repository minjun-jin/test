...
// @updateURL    https://raw.githubusercontent.com/minjun-jin/test/refs/heads/main/tampermonkey_enterjoy_point_clock_v124.user.js
// @downloadURL  https://raw.githubusercontent.com/minjun-jin/test/refs/heads/main/tampermonkey_enterjoy_point_clock_v124.user.js
// @version      1.24
...
(function () {
    ...
    // Prevent execution inside iframes as early as possible
    ...
    const POINT_LINKS = [
        ...
        'https://enterjoy.day/bbs/board.php?bo_table=point&wr_id=29484'
    ];
        ...
        if (event.data?.type === 'ej_point_update') {
            ...
            if (newTime) {
                ...
                if (timerObj) {
                    ...
                    if (timerObj.debugTimeLabel && timerObj.debugTimeLabel.textContent !== newTime) {
                ...
                if (timerObj) {
                    ...
                    if (timerObj.clockDisplay.textContent !== '--:--:--') {
                        timerObj.clockDisplay.textContent = '--:--:--';
                    }
                    if (timerObj.clockDisplay.style.backgroundColor !== 'rgba(0, 0, 0, 1)') {
                        timerObj.clockDisplay.style.backgroundColor = 'rgba(0, 0, 0, 1)';
                    }
                    if (timerObj.debugTimeLabel && timerObj.debugTimeLabel.textContent !== '') {
    ...
    const observer = new MutationObserver(() => {
        const currentHasClock = document.querySelector('#tampermonkey-synced-clock') !== null;
        const currentHasTimer = document.querySelector('#tampermonkey-synced-timer') !== null;

        if (hasClock !== currentHasClock || hasTimer !== currentHasTimer) {
            hasClock = currentHasClock;
            hasTimer = currentHasTimer;
            BASE_BOTTOM = getBaseBottom();

            // Reposition all timers only when base layout actually shifts
    ...
    function checkAndTriggerCountdownAlarms(timerObj, diff) {
        ...
        // Pre-alarm
        const isBeepPreTime = (flooredSeconds <= 60 && (flooredSeconds > 30));
        if (isBeepPreTime) {
            if (!timerObj.beepPreTriggered) {
                timerObj.beepPreTriggered = true;
            }
            const targetBg = (flooredSeconds % 2 === 0) ? 'rgba(255, 0, 0, 1)' : 'rgba(0, 0, 0, 1)';
            if (timerObj.clockDisplay.style.backgroundColor !== targetBg) {
                timerObj.clockDisplay.style.backgroundColor = targetBg;
            }
        } else {
        ...
        // Low alarm
        const isBeepLowTime = flooredSeconds <= 30 && flooredSeconds > 0;
        if (isBeepLowTime) {
            if (flooredSeconds % 2 === 0) {
        ...
        // High alarm
        const isBeepHighTime = flooredSeconds <= 0;
        if (isBeepHighTime) {
            if (!timerObj.beepHighTriggered) {
                timerObj.beepHighTriggered = true;
                playBeepHigh();
            }
            if (timerObj.clockDisplay.style.backgroundColor !== 'rgba(255, 0, 0, 1)') {
                timerObj.clockDisplay.style.backgroundColor = 'rgba(255, 0, 0, 1)';
            }
            return;
        } else {
    ...
    function updateCountdown(timerObj, targetTime) {
        ...
        if (diff <= 0) {
            if (timerObj.clockDisplay.textContent !== '00:00:00') {
            ...
            if (timerObj.clockDisplay.textContent !== timeString) {
                timerObj.clockDisplay.textContent = timeString;
            }
    ...
    function createDebugUI(timerObj, rowContainerOrBottom) {
        ...
        inputButton.onclick = () => {
            ...
            if (input && /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(input)) {
                ...
                if (debugTimeLabel.textContent !== input) {
    ...
    function observeRewardDiv(timerObj) {
        const rewardObserver = new MutationObserver((mutations) => {
            for (const mutation of mutations) {
                for (const node of mutation.addedNodes) {
                    try {
                        // Protect cross-origin nodes tracking using try-catch blocks
                        if (node && node.nodeType === 1 && node.id && node.id.startsWith('ej_reward_')) {
                            ...
                            if (targetTime) {
                                ...
                                syncChannel.postMessage({ type: 'ej_point_update', wr_id: timerObj.wr_id, targetTime });
                                if (timerObj.debugTimeLabel && timerObj.debugTimeLabel.textContent !== targetTime) {
                        ...
                        // Safe catch block against restricted compartment wrapper access
                    }
                }
            }
        });
        ...
        if (location.href.includes('bo_table=point') && location.href.includes(`wr_id=${wr_id}`)) {
            ...
            document.body.addEventListener('click', (e) => {
                try {
                    ...
                    // Guard against potential restricted element checks
                }
            });
        }
    });
